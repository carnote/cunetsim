...
__global__ void Init_Rem_Comm_Buffer(struct RemoteCommBuffer *rem_comm_buffer, int node_number) {
	int tid = blockDim.x * blockIdx.x + threadIdx.x;
	if (tid < node_number) {
		rem_comm_buffer[tid].write_index = 0;
		rem_comm_buffer[tid].read_index = -1;
		for (int i = 0; i < Maxelement; i++) {
			rem_comm_buffer[tid].element[i].header[0] = -1;
			rem_comm_buffer[tid].element[i].header[1] = -1;
			rem_comm_buffer[tid].element[i].header[2] = -1;
			rem_comm_buffer[tid].element[i].header[3] = -1;
		}
	}
}
