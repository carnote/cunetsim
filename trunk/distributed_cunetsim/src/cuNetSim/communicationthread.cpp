...
{
    int nread;
    int counter = 400 + nb_tours;
    Data_Flow_Unit data;
    Final_Data final;

    while (counter > 0) {

        switch( nread = read(tube_descriptor, &data, sizeof(struct Data_Flow_Unit))) {
        case -1 :
            perror( "read" );
            break;
        case 0 :
        ...
        default :
            ...
            if (counter % 5 == 0) {
                for (int i = 0; i<NB_DISPLAYED_NODES; i++){
                    geo[i].neighbor_number = data.geo[i].neighbor_number;
                    geo[i].p = data.geo[i].p;
                    for(int j = 0; j<data.geo[i].neighbor_number; j++)
                        geo[i].neighbor_vector[j] = data.geo[i].neighbor_vector[j];
                }
            }
            for (int i = 0; i < NB_DISPLAYED_NODES; i++)
    ...
    switch(nread = read(tube_descriptor, &final, sizeof(Final_Data))){
        case -1 :
            perror( "read" );
            break;
        case 0 :
