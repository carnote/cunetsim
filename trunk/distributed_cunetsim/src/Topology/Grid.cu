	...
	*host_total_dest = 0;
	...
	for (int i = 0; i < nb_tours + partition.additional_time; i++) {
...

		{
			...
			Sender<<<threads,grid>>>(device_geo,device_in_phy,device_out_phy, node_number, device_forwarded_per_node, NULL, device_new_message, offset, device_remote_comm_out, m_send, b_send);
