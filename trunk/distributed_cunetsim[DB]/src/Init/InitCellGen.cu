        ...
        for(i=-1;i<2;i++)//x
        {
            for(j=-1;j<2;j++)//y
            {
                for(k=-1;k<2;k++)//z
                {
                    ...
                    {
			int cell_neighbor;
			for (cell_neighbor = 0; cell_neighbor < cell[id].neighbor_number; cell_neighbor++)
				if (cell[id].neighbor_vector[cell_neighbor] == id+i+j*step_x+k*step_x*step_y)
					break;
			if (cell_neighbor == cell[id].neighbor_number) {
