...
struct Performances Random(int node_number, int f, float drop_prob, void *genV, int write_descriptor);
...
__host__ void Init_geo_static_grid(struct Geo *host_geo, int node_number);
...
__host__ void Init_geo(struct Cell *cell, struct Geo *geo, float *posrandx, float *posrandy,float *posrandz,float *vrandx,\
		float *vrandy,float *vrandz, int node_number);
...
__host__ void Init_cell_gen_2D(struct Cell *cell, int step_x, int step_y, int node_number);
__host__ void Init_cell_gen(struct Cell *cell, int step_x, int step_y, int step_z, int node_number);
...
__global__ void Receiver(struct Buffer *in_phy_buff, int node_number, float m_recv, float b_recv, struct Geo *geo);
...
__global__ void Application_Out(MessageBuffer *app_out_buff, int *senders_receivers, int message_digit, int node_number, int tour, int nb_tours);
...
__global__ void Init_Cluster(struct Cluster *cluster);

__global__ void Clustering(struct Geo *geo, struct Cluster *cluster, int node_number);
__global__ void Clustering2(struct Geo *geo, struct Cluster *cluster, int node_number);
__global__ void Clustering3(struct Geo *geo, struct Cluster *cluster, int node_number);
