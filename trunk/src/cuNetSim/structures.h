...
typedef struct Geo {
        ...
        int CellId; // id of the cell the node is currently visiting
        int OldCellId; // id of the former cell it visited
        int CellPosition; // ?
        int Viewed; // ?
        int Neighbors; // number of neighboring nodes (distance between the node and its neighbors < 100)
        int Neighbor[Maxneighbor]; // array of its neighbors
	float energy;
} Geo;
